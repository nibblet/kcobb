#!/usr/bin/env python3
"""Cluster PRINCIPLES only from out/extracted/*.json. Fingerprint dedup, frequency, human labels.
Outputs: out/clusters/principles_*.json|.md, out/doctrine/draft_principles_doctrine.md, core_principles.md."""
...
OUT_LEGACY = BASE / "out" / "legacy"
K_MIN, K_MAX = 2, 18
N_REP = 5
TOP_N_CORE = 20
# Top N clusters by count that should have human_label set before considering doctrine "final"
TOP_N_CLUSTERS_REQUIRE_LABEL = 15
LEGACY_ARTIFACTS = ["clusters.json", "cluster_summary.md", "draft_doctrine.md", "cluster_items.csv", "dedup_map.json"]
...
def main() -> None:
    ...
    # Move legacy combined artifacts to out/legacy/ (single source of truth: principles_* and heuristics_*)
    OUT_LEGACY.mkdir(parents=True, exist_ok=True)
    for name in LEGACY_ARTIFACTS:
        p = OUT_CLUSTERS / name
        if p.exists():
            p.rename(OUT_LEGACY / name)

    raw_items: list[dict] = []
    ai_merged_path = OUT_CLUSTERS / "ai_merged_principles.json"
    if ai_merged_path.exists():
        try:
            canonical = json.loads(ai_merged_path.read_text(encoding="utf-8"))
            if not canonical:
                canonical = []
            print("[dim]Using ai_merged_principles.json (skipping fingerprint_dedup + near_duplicate_merge)[/dim]")
    ...
    if canonical is None:
        raw_items = load_extracted_items(IN_DIR, "principle", "principle")
        if not raw_items:
            print("[yellow]No principles found in out/extracted/*.json[/yellow]")
            return
        canonical = fingerprint_dedup(raw_items, "principle")
        if not canonical:
            print("[yellow]No canonical principles after dedup.[/yellow]")
            return
    ...
    merge_labels_editable(OUT_CLUSTERS, "principles", summaries)
    ...
    if labels_editable_path.exists():
        try:
            ...
            labels_editable = raw.get("principles", {}) or {}
        except Exception:
            pass
    ...
    # Check if top clusters have human_label (for final doctrine)
    ...
    (OUT_CLUSTERS / "principles_clusters.json").write_text(
        json.dumps(summaries, indent=2), encoding="utf-8"
    )
    lines = ["# Principle Clusters\n"]
    for s in sorted(summaries, key=lambda x: -x["count"]):
        ...
        lines.append("")
    (OUT_CLUSTERS / "principles_cluster_summary.md").write_text("\n".join(lines), encoding="utf-8")

    # Draft doctrine: by cluster label, frequency desc; each item with statement, frequency, story_ids, one evidence
    draft_lines = ["# Draft Principles Doctrine\n", doctrine_note, "Sorted by frequency; grouped by cluster. Headers marked (needs rename) until human_label is set.\n"]
    for s in sorted(summaries, key=lambda x: -x["count"]):
        label = cluster_label(s["cluster_id"])
        ...
        for it in items_sorted:
            ...
            draft_lines.append("")
        draft_lines.append("")
    (OUT_DOCTRINE / "draft_principles_doctrine.md").write_text("\n".join(draft_lines), encoding="utf-8")

    # core_principles.md: top N by frequency across all canonical
    all_canonical: list[dict] = []
    for s in summaries:
        for it in s["canonical_items"]:
            all_canonical.append({**it, "cluster_label": cluster_label(s["cluster_id"])})
    all_canonical.sort(key=lambda x: (-x["frequency"], -x.get("total_mentions", 0)))
    core_lines = ["# Core Principles\n", doctrine_note, f"Top {TOP_N_CORE} by frequency (stories supporting each idea).\n"]
    for it in all_canonical[:TOP_N_CORE]:
        core_lines.append(f"- {it['display_text']}  _({it['frequency']} stories: {', '.join(it['story_ids'])})_")
    (OUT_DOCTRINE / "core_principles.md").write_text("\n".join(core_lines), encoding="utf-8")

    raw_count = len(raw_items) if raw_items else len(canonical)
    print(f"[bold green]Principles: {raw_count} raw -> {len(canonical)} canonical -> {k} clusters.[/bold green]")
    print(f"  Outputs: {OUT_CLUSTERS}/principles_clusters.json, principles_cluster_summary.md")
    print(f"  Doctrine: {OUT_DOCTRINE}/draft_principles_doctrine.md, core_principles.md")
